...
u8 Dfs_decode_init(u8* fname,__wavctrl* wavx)
{
	FIL*ftemp;
	u8 *buf; 
	u32 br=0;
...
//bits:位数(16/24)
//返回值:读到的数据个数
u32 dfs_buffill(u8 *buf,u16 size,u8 bits)
{
	u16 readlen=0;
	u32 bread;
	u16 i;
...
void dfs_i2s_dma_tx_callback(void) 
{   
	u16 i;
	#if     RED_DRAGONBOARD
...
//fx:文件指针
//wavx:wav播放控制器
void dfs_get_curtime(FIL*fx,__wavctrl *wavx)
{
	long long fpos;  	
...
u8 dfs_play_song(u8* fname)
{
	...
	#if RED_DRAGONBOARD
				I2S3_Init(I2S_Standard_Phillips,I2S_Mode_MasterTx,I2S_CPOL_Low,I2S_DataFormat_16bextended);
	#elseif  DSD_SUPPORT
	...
	#if RED_DRAGONBOARD
				I2S3_Init(I2S_Standard_Phillips,I2S_Mode_MasterTx,I2S_CPOL_Low,I2S_DataFormat_24b);
	#elseif  DSD_SUPPORT
	...
	#if RED_DRAGONBOARD
				I2S3_Init(I2S_Standard_Phillips,I2S_Mode_MasterTx,I2S_CPOL_Low,I2S_DataFormat_32b);
	#elseif  DSD_SUPPORT
				I2S2_Init(I2S_Standard_Phillips,I2S_Mode_MasterTx,I2S_CPOL_Low,I2S_DataFormat_32b);		//飞利浦标准,主机发送,时钟低电平有效,24位扩展帧长度
	#endif
			}
	#if RED_DRAGONBOARD		
