...
u8 wav_decode_init(u8* fname,__wavctrl* wavx)
{
 ...
 u8 tbuf[WAV_I2S_TX_DMA_BUFSIZE];
//u8  xbuf[WAV_I2S_TX_DMA_BUFSIZE];
//填充buf
//buf:数据区
//size:填充数据量
//bits:位数(16/24)
...
u8 wav_play_song(u8* fname)
{
	...
	{ 
		...
		if(res==0)//解析文件成功
		{
			if(wavctrl.bps==16)
			{
				iic_flag=AK4495_I2S_Cfg(0x00,16,wavctrl.samplerate);	//飞利浦标准,16位数据长度
				I2S3_Init(I2S_Standard_LSB,I2S_Mode_MasterTx,I2S_CPOL_Low,I2S_DataFormat_16b);
			}
			else if(wavctrl.bps==24)
			{
				iic_flag=AK4495_I2S_Cfg(0x00,24,wavctrl.samplerate);	//飞利浦标准,24位数据长度
				I2S3_Init(I2S_Standard_Phillips,I2S_Mode_MasterTx,I2S_CPOL_Low,I2S_DataFormat_24b);
			}
			else if(wavctrl.bps==32)
			{
				if(wavctrl.audioformat==1)
				{
					iic_flag=AK4495_I2S_Cfg(0x00,32,wavctrl.samplerate);	//飞利浦标准,32位数据长度
					I2S3_Init(I2S_Standard_LSB,I2S_Mode_MasterTx,I2S_CPOL_Low,I2S_DataFormat_32b);
				}
        else
				{
					NOKIA5110_clear();
					NOKIA5110_write_english_string(1,0,"float32bit not supported");
					return 0xff;
				}					
			}	
