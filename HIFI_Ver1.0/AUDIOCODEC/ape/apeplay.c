...
u8 ape_play_song(u8* fname) 
{
	...
	{   
		...
		printf("  Bitrate: %d kbps\r\n",apectrl->bitrate); 
 		//ape_dumpinfo(apex);
 		iic_flag=AK4495_I2S_Cfg(0x00,16,apectrl->samplerate);	//飞利浦标准,16位数据长度
		I2S3_Init(I2S_Standard_LSB,I2S_Mode_MasterTx,I2S_CPOL_Low,I2S_DataFormat_16b);		//飞利浦标准,主机发送,时钟低电平有效,16位扩展帧长度
		I2S3_SampleRate_Set(apex->samplerate);		//设置采样率 
		I2S3_TX_DMA_Init(audiodev.i2sbuf1,audiodev.i2sbuf2,APE_BLOCKS_PER_LOOP*2);//配置TX DMA
