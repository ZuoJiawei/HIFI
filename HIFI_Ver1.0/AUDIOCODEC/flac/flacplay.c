...
u8 flac_init(FIL* fx,__flacctrl* fctrl,FLACContext* fc)
{
	...
	while(!endofmetadata) 
	{
		...
		{ 
			...
			fc->min_framesize=fc->min_blocksize*8;//((u32)buf[4]<<16)|((u16)buf[5]<<8)|buf[6];//最小帧大小
			fc->max_framesize=fc->max_blocksize*8;//((u32)buf[7]<<16)|((u16)buf[8]<<8)|buf[9];//最大帧大小
...
{ 
	...
	{ 
		...
		{
			iic_flag=AK4495_I2S_Cfg(0x00,24,flacctrl->samplerate);	//飞利浦标准,24位数据长度
			I2S3_Init(I2S_Standard_Phillips,I2S_Mode_MasterTx,I2S_CPOL_Low,I2S_DataFormat_24b);	//飞利浦标准,主机发送,时钟低电平有效,24位扩展帧长度
		...
		{
			iic_flag=AK4495_I2S_Cfg(0x00,16,flacctrl->samplerate);	//飞利浦标准,16位数据长度
			I2S3_Init(I2S_Standard_LSB,I2S_Mode_MasterTx,I2S_CPOL_Low,I2S_DataFormat_16b);	//飞利浦标准,主机发送,时钟低电平有效,16位扩展帧长度	
		...
		I2S3_SampleRate_Set(fc->samplerate);		//设置采样率 
